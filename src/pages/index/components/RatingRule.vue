<template>
  <div>
    <div style="z-index: 10;position: relative">
      ...
      <p>1、所有队伍初始得分为1500</p>
      <p>2、每场比赛后计算一个排名得分S （ S=（比赛得分线性归一化后）*比赛队伍数 ）（线性归一化函数: x = (x-min)/(max-min)）</p>
      <p>3、每场比赛后计算一个期望得分E （ E=sum(1/(1+10^((Rb-Ra)/400)))</p>
      <p>其中Ra代表你当前积分，Rb代表每一队伍的得分</p>
      <p>4、为了鼓励思考，计算一个系数K，K=（本次比赛得分/平均分）</p>
      <p>5、为了鼓励新人，计算一个系数P，P=（开始比赛前的积分排名/总人数）</p>
      <p>6、计算积分的变化 D = 20*(K+P+S-E)</p>
      <el-table
              :data="RatingData"
              :row-style="rankcolor"
              border
      ...
      >
        ...
        <el-table-column prop="rating" label="Score"/>
        <el-table-column prop="color" label="Color"/>
      </el-table>

      <h3>每场比赛排名怎么算?</h3>
      <h3>个人赛：</h3>
      <p>每道题目有一个积分，每分钟衰减(0.7/i)%，i为题号 1~26,比赛一般为120分钟。每道题分数按顺序为1000 1500 1600 2000 2000 2000往后都为2000</p>
      <p>每一次失败提交-20分，一血加分为 题目积分/10</p>
      <br>
      <p>集训组队赛：</p>
      <p>为减轻管理员负担，并鼓励积极思考，按照如下规则算分，所有队伍仅算校内队伍</p>
      <p>每道题初始分为500+500*(比赛总队伍-AC队数)</p>
      <p>每道题目有一个积分，按照AC顺序得分为 题目积分-(题目积分/做出总人数/2)*(AC顺序-1)</p>
      <p>每一次失败提交-100分（每道题目最多算十发罚时），一血加分为 题目积分/100</p>
    </div>
    <MountainFooter/>
  </div>
</template>
